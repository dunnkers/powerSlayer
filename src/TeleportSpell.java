...
enum TeleportSpell implements Teleport {
    LUMBRIDGE_HOME(Magic.SPELL_HOME_TELEPORT, new RSTile(0, 0, 0), 0),
    VARROCK(Magic.SPELL_VARROCK_TELEPORT, new RSTile(0, 0, 0), 25, Rune.FIRE, 3, Rune.AIR, Rune.LAW),
    MOBILISING_ARMIES(Magic.SPELL_MOBILISING_ARMIES_TELEPORT, new RSTile(0, 0, 0), 10, Rune.LAW, Rune.WATER, Rune.AIR),
    LUMBRIDGE(Magic.SPELL_LUMBRIDGE_TELEPORT, new RSTile(0, 0, 0), 31, Rune.LAW, Rune.AIR, 3, Rune.EARTH),
    FALADOR(Magic.SPELL_FALADOR_TELEPORT, new RSTile(0, 0), 37, Rune.LAW, Rune.AIR, 3, Rune.WATER),
    CAMELOT(Magic.SPELL_CAMELOT_TELEPORT, new RSTile(0, 0), 45, Rune.LAW, Rune.AIR, 5);
    private RSTile loc;
    ...
    private TeleportSpell(int spellNum, RSTile loc, int magicLevel,
        ...
        for (Rune r : runes) {
            ...
            this.runes.put(r, (oldVal != null ? oldVal : 0) + 1);
        }
    }
        ...
        for (int i = 0; i < runes.length; i++) {
            if (runes[i] instanceof Rune) {
                ...
                if (i + 1 < runes.length && runes[i + 1] instanceof Integer) {
                    count = (Integer) runes[i + 1];
                }
                Integer old = this.runes.get(rune);
                this.runes
                        .put(rune, (old != null ? old : 0) + count);
            }
        }
    }
